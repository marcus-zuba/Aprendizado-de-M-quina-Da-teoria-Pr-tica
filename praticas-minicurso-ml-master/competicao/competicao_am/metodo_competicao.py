...
import pandas as pd
from preprocessamento_atributos_competicao import gerar_atributos_ator, gerar_atributos_resumo, gerar_atributos_titulo
...
class MetodoCompeticao(MetodoAprendizadoDeMaquina):
    ...
    def eval_titulo(self, df_treino:pd.DataFrame, df_data_to_predict:pd.DataFrame, col_classe:str, seed:int=1):
        #separação da classe 
        x_treino, x_to_predict = self.obtem_x(df_treino, df_data_to_predict, col_classe)
        y_treino, y_to_predict = self.obtem_y(df_treino, df_data_to_predict, col_classe)
        
        #geração dos atributos por meio do df_treino e df_data_to_predict
        df_treino_titulo, df_to_predict_titulo = gerar_atributos_titulo(x_treino, x_to_predict)
        
        self.ml_method.fit(df_treino_titulo, y_treino)
        arr_predict = self.ml_method.predict(df_to_predict_titulo)
    ...
    def combine_predictions(self, arr_predictions_1:List[int], arr_predictions_2:List[int], arr_predictions_3:List[int]) -> List[int]:
        #realiza a predicao final
        
        y_final_predictions = []
        
        #COMBINAR PREDIÇÕES POR MAIORIA
        
        for i in range(len(arr_predictions_1)):
            
            if arr_predictions_1[i] == arr_predictions_2[i]:
                y_final_predictions.append(arr_predictions_1[i])
            
            elif arr_predictions_1[i] == arr_predictions_3[i]:
                y_final_predictions.append(arr_predictions_1[i])
            
            elif arr_predictions_2[i] == arr_predictions_3[i]:                
